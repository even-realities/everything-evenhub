...
| `bridge.updateImageRawData(data)` | `Promise<ImageRawDataUpdateResult>` | Send raw pixel data to fill an image container. LZ4-compressed internally by the SDK (0.0.12+) — pass raw pixels as before. Calls must be serial — await each before the next — and are paced at 100ms since 0.0.14. |
...
6. **Image sends must be serial, and are paced at 100ms (0.0.14+)** — `updateImageRawData` calls must be queued and awaited one at a time; concurrent calls are not supported and will cause errors. Since 0.0.14 each send also holds the image path for 100ms — a call inside that window is held and flushed on the next one, so nothing is dropped and nothing arrives early. Treat 100ms as a floor, not a frame budget; a single frame costs far longer over BLE. Keep checking `ImageRawDataUpdateResult`, but read it as a call result rather than a delivery receipt from the glasses.
